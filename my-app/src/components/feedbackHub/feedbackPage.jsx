...
const REPORT_REASONS = [
  ...
  { value: "LOW_EFFORT",   label: "Too short or generic" },
  { value: "OFF_TOPIC",    label: "Doesn't address my work" },
  { value: "OFFENSIVE",    label: "Rude or inappropriate" },
...
function AuthorChip({ user, size = "md" }) {
  ...
  return (
    <Link to={`/profile/${user?.id}`} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
      <div className={`${sz} rounded-full bg-[#2d3748] flex items-center justify-center text-white font-semibold flex-shrink-0 overflow-hidden`}>
        {user?.avatar
...
// ─── SMALL UPVOTE BUTTON (paragraph comments) ────────────────────────────────
...
function ReportModal({ responseId, onClose, onSuccess }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="px-6 py-5 border-b border-[#f0ebe3] flex items-center justify-between">
          <div>
            <h3 className="font-serif text-lg text-[#2d3748]">Report this critique</h3>
            <p className="text-xs text-[#9a8c7a] mt-0.5">Your report goes to the Inkwell admin for review.</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg hover:bg-[#f4f1ec] flex items-center justify-center text-[#9a8c7a] hover:text-[#2d3748] transition-colors"
          >
        ...
        <div className="px-6 py-5 space-y-4">
            ...
            {REPORT_REASONS.map((r) => (
              <label
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-all ${
                  reason === r.value
                    ? "border-[#2d3748] bg-[#f4f1ec]"
                    : "border-[#e8e0d0] hover:border-[#c4b9ab] bg-white"
                }`}
              >
                <input
                  type="radio"
                ...
                <span className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${
                  reason === r.value ? "border-[#2d3748]" : "border-[#c4b9ab]"
                }`}>
                  {reason === r.value && <span className="w-2 h-2 rounded-full bg-[#2d3748]" />}
                </span>
                <span className="text-sm text-[#2d3748]">{r.label}</span>
              </label>
            ))}
          </div>
          ...
          {reason === "OTHER" && (
            <textarea
              ...
              className="w-full border border-[#e8e0d0] rounded-xl px-4 py-3 text-sm text-[#2d3748] placeholder-[#c4b9ab] bg-[#faf7f2] focus:outline-none focus:border-[#2d3748] resize-none"
            />
          )}

          {error && (
            <p className="text-sm text-[#c0392b] bg-[#fdf1f0] border border-[#f5c6c3] rounded-xl px-4 py-3">
              {error}
            </p>
          )}
        </div>

        <div className="px-6 py-4 border-t border-[#f0ebe3] flex items-center justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm text-[#9a8c7a] hover:text-[#2d3748] transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-5 py-2 bg-[#c0392b] text-white text-sm font-semibold rounded-xl hover:bg-[#9b1c1c] transition-all disabled:opacity-40"
          >
...
function CommentSidebar({
  submissionId, paragraphIndex, paragraphPreview, comments,
  onClose, onNewComment, onUpvoteComment, onEditComment, onDeleteComment,
  isAuthor, user,
}) {
  ...
  async function submitComment() {
    ...
    try {
      ...
      if (res.ok) { const data = await res.json(); onNewComment(data); setCommentText(""); }
    } catch (e) {}
    setPosting(false);
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#f0ebe3] flex-shrink-0">
        ...
        <button onClick={onClose} className="flex-shrink-0 w-7 h-7 rounded-lg hover:bg-[#f4f1ec] flex items-center justify-center text-[#9a8c7a] hover:text-[#2d3748] transition-colors">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
        {comments.length === 0 && (
          <p className="text-sm text-[#b8a898] text-center py-8">No comments on this paragraph yet.</p>
        )}
        {comments.map((c) => {
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1.5 mb-1.5">
                    ...
                    {isOwn && !isEditing && (
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => { setEditingId(c.id); setEditText(c.content); }} className="text-[10px] text-[#9a8c7a] hover:text-[#2d3748] px-1.5 py-0.5 rounded transition-colors">Edit</button>
                        <button onClick={() => deleteComment(c.id)} disabled={deletingId === c.id} className="text-[10px] text-[#c0392b] hover:text-[#9b1c1c] px-1.5 py-0.5 rounded transition-colors disabled:opacity-40">Delete</button>
                      </div>
                    )}
                  </div>
                  {isEditing ? (
                    <div className="space-y-2">
                      <textarea value={editText} onChange={(e) => setEditText(e.target.value)} rows={3} autoFocus className="w-full border border-[#2d3748] rounded-lg px-3 py-2 text-xs text-[#2d3748] bg-white focus:outline-none resize-none leading-relaxed" />
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => { setEditingId(null); setEditText(""); }} className="text-[11px] text-[#9a8c7a] hover:text-[#2d3748] px-2 py-1 transition-colors">Cancel</button>
                        <button onClick={() => saveEdit(c.id)} disabled={editSaving || !editText.trim()} className="px-3 py-1 bg-[#2d3748] text-white text-[11px] font-semibold rounded-lg disabled:opacity-40 hover:opacity-90 transition-all">{editSaving ? "Saving..." : "Save"}</button>
                      </div>
                    </div>
                  ) : (
                  ...
                  {!isEditing && (
                    <div className="flex items-center gap-2 mt-2.5">
                      <CommentUpvoteButton count={upvCount} upvoted={upvoted} onToggle={() => onUpvoteComment(c.id)} disabled={!user || isOwn} />
                      {user && (
                        <button onClick={() => openReply(c.id)} className="text-[11px] text-[#9a8c7a] hover:text-[#2d3748] transition-colors font-medium">
                        ...
                        <div key={r.id} className="group/reply">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            <Link to={`/profile/${r.author?.id}`} className="w-5 h-5 rounded-full bg-[#6b5c4a] flex items-center justify-center text-white text-[9px] font-semibold flex-shrink-0 overflow-hidden hover:opacity-80 transition-opacity">
                              {r.author?.avatar ? <img src={r.author.avatar} alt={r.author.username} className="w-full h-full object-cover" /> : <span>{r.author?.username?.charAt(0).toUpperCase() ?? "?"}</span>}
                            </Link>
                          ...
                          {user && (
                            <button onClick={() => openReply(c.id, { username: r.author?.username })} className="ml-7 mt-1 text-[10px] text-[#b8a898] hover:text-[#2d3748] transition-colors font-medium">Reply</button>
                          )}
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="mt-3 pl-3 border-l-2 border-[#e8e0d0]">
                      {replyingTo && (
                        <div className="flex items-center gap-1.5 mb-1.5">
                          ...
                          <button onClick={() => { setReplyingTo(null); setReplyText(""); }} className="text-[10px] text-[#b8a898] hover:text-[#c0392b] ml-1 transition-colors">✕</button>
                        </div>
                      )}
                      <textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} rows={2} autoFocus placeholder={replyingTo ? `Reply to @${replyingTo.username}...` : "Write a reply..."} className="w-full border border-[#e8e0d0] rounded-xl px-3 py-2 text-xs text-[#2d3748] placeholder-[#c4b9ab] bg-[#faf7f2] focus:outline-none focus:border-[#2d3748] resize-none transition-all" />
                      <div className="flex items-center justify-between mt-1.5">
                        <button onClick={() => { setReplyOpen(null); setReplyText(""); setReplyingTo(null); }} className="text-[11px] text-[#9a8c7a] hover:text-[#2d3748] transition-colors">Cancel</button>
                        <button onClick={() => submitReply(c.id)} disabled={replyPosting || !replyText.trim()} className="px-3 py-1.5 bg-[#2d3748] text-white text-xs rounded-lg disabled:opacity-40 hover:opacity-90 transition-all font-medium">{replyPosting ? "Posting..." : "Reply"}</button>
                      </div>
                    </div>
                  )}
                </div>
      ...
      {user && (
        <div className="px-5 py-4 border-t border-[#f0ebe3] flex-shrink-0 bg-white">
          <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)} rows={3} placeholder="Add a comment on this paragraph..." className="w-full border border-[#e8e0d0] rounded-xl px-3 py-2.5 text-sm text-[#2d3748] placeholder-[#c4b9ab] bg-[#faf7f2] focus:outline-none focus:border-[#2d3748] focus:ring-2 focus:ring-[#2d3748]/10 resize-none transition-all" />
          <div className="flex justify-end mt-2">
            <button onClick={submitComment} disabled={posting || !commentText.trim()} className="px-4 py-2 bg-[#2d3748] text-white text-sm rounded-xl disabled:opacity-40 hover:opacity-90 transition-all font-semibold">{posting ? "Posting..." : "Comment"}</button>
          </div>
        </div>
      )}
    </div>
...
function WordCountBar({ current, min, label }) {
  const pct = Math.min((current / min) * 100, 100);
  const met = current >= min;
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label className="text-sm font-semibold text-[#2d3748]">{label}</label>
        <span className={`text-xs font-semibold transition-colors ${met ? "text-[#6b8c6b]" : current > 0 ? "text-[#b8860b]" : "text-[#b8a898]"}`}>
          {current} words
        </span>
      </div>
      <div className="h-1 bg-[#f0ebe3] rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-300 ${met ? "bg-[#6b8c6b]" : "bg-[#d4af37]"}`} style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
}
...
function CritiqueForm({ submissionId, minWords, feedbackWanted, onSuccess }) {
  const [whatWorking,    setWhatWorking]    = useState("");
  const [whatNeedsWork,  setWhatNeedsWork]  = useState("");
  const [selfWritten,    setSelfWritten]    = useState(false);
  const [submitting,     setSubmitting]     = useState(false);
  const [error,          setError]          = useState("");

  const workingWords   = countWords(whatWorking);
  const needsWorkWords = countWords(whatNeedsWork);
  const totalWords     = workingWords + needsWorkWords;
  const wordsMet       = totalWords >= minWords;
  const canSubmit      = wordsMet && selfWritten && whatWorking.trim() && whatNeedsWork.trim();

  async function handleSubmit() {
    if (!whatWorking.trim())   { setError("Please fill in what's working."); return; }
    if (!whatNeedsWork.trim()) { setError("Please fill in what needs improvement."); return; }
    if (!wordsMet) {
      setError(`Your critique needs at least ${minWords} words total. You have ${totalWords}.`);
      return;
    }
    if (!selfWritten) {
      setError("Please confirm you wrote this critique yourself.");
      return;
    }

    // Combine into structured generalFeedback the backend already understands
    ...
    try {
      const res = await fetch(`${API_URL}/feedback/submissions/${submissionId}/responses`, {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ generalFeedback }),
      });
      const data = await res.json();
  ...
  return (
    <div className="bg-white border border-[#e8e0d0] rounded-2xl overflow-hidden shadow-[0_2px_12px_rgba(45,35,20,0.04)]">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-[#7a5c1e] leading-relaxed">
              ...
              <a href="https://discord.gg/KsZjEjjsh" target="_blank" rel="noopener noreferrer" className="font-semibold underline underline-offset-2 hover:text-[#92680a] transition-colors">Discord server</a>.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        {/* Trust appeal */}
        <p className="text-sm text-[#6b5c4a] leading-relaxed mb-5 border-l-2 border-[#d4af37] pl-4">
          This writer shared their work trusting it would be read with care. They spent hours on this chapter — give them the honest, human feedback only a real reader can offer.
        </p>

        {/* feedbackWanted reminder */}
        {feedbackWanted?.length > 0 && (
          <div className="mb-6 bg-[#f2f0fd] border border-[#d5d0f7] rounded-xl px-4 py-3.5">
            <p className="text-xs font-semibold text-[#6558d4] uppercase tracking-wide mb-2">
              The writer specifically wants feedback on
            </p>
            <div className="flex flex-wrap gap-2">
              {feedbackWanted.map((tag, i) => (
                <span key={i} className="text-xs text-[#6558d4] bg-white border border-[#d5d0f7] px-3 py-1 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
        ...
        {/* Combined word count progress */}
        <div className="mb-5">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs text-[#9a8c7a]">Combined word count</span>
            <span className={`text-xs font-semibold transition-colors ${wordsMet ? "text-[#6b8c6b]" : totalWords > 0 ? "text-[#b8860b]" : "text-[#b8a898]"}`}>
              {totalWords} / {minWords} words minimum
            </span>
          </div>
          <div className="h-1.5 bg-[#f0ebe3] rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-300 ${wordsMet ? "bg-[#6b8c6b]" : "bg-[#d4af37]"}`}
              style={{ width: `${Math.min((totalWords / minWords) * 100, 100)}%` }}
            />
          </div>
        </div>
        ...
        <div className="mb-5">
          ...
          <textarea
            ...
            rows={6}
            placeholder="What did this writer do well? Be specific — name a scene, a line, a character moment, an image that landed. Generic praise ('good writing!') doesn't help."
            className="mt-2 w-full border border-[#e8e0d0] rounded-xl px-4 py-3.5 text-sm text-[#2d3748] placeholder-[#c4b9ab] bg-[#faf7f2] focus:outline-none focus:border-[#2d3748] focus:ring-2 focus:ring-[#2d3748]/10 transition-all resize-none leading-relaxed"
          />
        </div>
        ...
        <div className="mb-7">
          ...
          <textarea
            ...
            rows={6}
            placeholder="What could be stronger? Point to specific moments — a line that confused you, a pace that dragged, a character choice that didn't land. Explain why, not just what."
            className="mt-2 w-full border border-[#e8e0d0] rounded-xl px-4 py-3.5 text-sm text-[#2d3748] placeholder-[#c4b9ab] bg-[#faf7f2] focus:outline-none focus:border-[#2d3748] focus:ring-2 focus:ring-[#2d3748]/10 transition-all resize-none leading-relaxed"
          />
        </div>

        {/* Self-written checkbox */}
        <label className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all mb-6 ${
          selfWritten ? "border-[#2d3748] bg-[#f4f1ec]" : "border-[#e8e0d0] bg-white hover:border-[#c4b9ab]"
        }`}>
          <div className={`mt-0.5 w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center transition-all ${
            selfWritten ? "bg-[#2d3748] border-[#2d3748]" : "border-[#c4b9ab]"
          }`}>
            {selfWritten && (
          ...
          <input type="checkbox" className="sr-only" checked={selfWritten} onChange={(e) => { setSelfWritten(e.target.checked); setError(""); }} />
          <div>
            <p className="text-sm font-semibold text-[#2d3748]">I wrote this critique myself</p>
            <p className="text-xs text-[#9a8c7a] mt-0.5 leading-relaxed">
              I engaged with this work genuinely and did not use AI to generate or substantially rewrite my feedback.
            </p>
          </div>
        </label>
        ...
        <button
          ...
          className="w-full py-3 bg-[#2d3748] text-white rounded-xl font-semibold text-sm hover:opacity-90 transition-all disabled:opacity-40 flex items-center justify-center gap-2"
        >
          {submitting ? (
            <>
            ...
            ? `${minWords - totalWords} more words needed`
            : !selfWritten
              ? "Confirm you wrote this to submit"
              : "Submit critique"}
        </button>
      </div>
    </div>
...
function CritiqueCard({ response, submissionAuthorId, user, onUpvote, onEdit, onDelete, onReport }) {
  ...
  const alreadyReported    = (response.reports?.length ?? 0) > 0;
  ...
  // Parse the stored generalFeedback back into the two sections for editing
  function startEdit() {
    const src = response.generalFeedback ?? "";
    const match = src.match(/\[WHAT'S WORKING\]([\s\S]*?)(?:\[WHAT NEEDS IMPROVEMENT\]([\s\S]*))?$/i);
    if (match) {
      setEditWorking(match[1]?.trim() ?? "");
      setEditNeedsWork(match[2]?.trim() ?? "");
    } else {
      // Legacy single-field critiques: put everything in the first box
      setEditWorking(src);
      setEditNeedsWork("");
    }
    setEditing(true);
  ...
  const editWorkingWords  = countWords(editWorking);
  const editNeedsWords    = countWords(editNeedsWork);
  const editTotal         = editWorkingWords + editNeedsWords;

  async function saveEdit() {
    if (!editWorking.trim()) { setEditError("Please fill in what's working."); return; }
    if (!editNeedsWork.trim()) { setEditError("Please fill in what needs improvement."); return; }
    const generalFeedback =
      `[WHAT'S WORKING]\n${editWorking.trim()}\n\n[WHAT NEEDS IMPROVEMENT]\n${editNeedsWork.trim()}`;
    setEditSaving(true);
    setEditError("");
    try {
      ...
      onEdit(response.id, { generalFeedback });
      setEditing(false);
    } catch (e) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (res.ok) onDelete(response.id);
    } catch (e) {}
    setDeleting(false);
    ...
    <>
      {reportOpen && (
        <ReportModal
          ...
          onSuccess={() => { setReportOpen(false); onReport(response.id); }}
        />
      )}
      ...
      <div className="bg-white border border-[#e8e0d0] rounded-2xl overflow-hidden shadow-[0_1px_6px_rgba(45,35,20,0.04)]">
        ...
        <div className="flex items-start justify-between gap-3 px-6 py-5 border-b border-[#f0ebe3]">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {canUpvote && (
              <ThumbUpButton count={upvCount} upvoted={upvoted} onToggle={() => onUpvote(response.id)} disabled={!user} />
            )}
            ...
            {user && !isCritiqueAuthor && (
              alreadyReported ? (
                ...
                <button
                  ...
                  title="Report this critique"
                  className="inline-flex items-center gap-1 text-[11px] text-[#9a8c7a] hover:text-[#c0392b] border border-[#e8e0d0] hover:border-[#f5c6c3] hover:bg-[#fdf1f0] px-2.5 py-1 rounded-lg transition-all"
                >
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isCritiqueAuthor && !editing && (
              <div className="flex items-center gap-1">
                <button onClick={startEdit} className="text-xs text-[#9a8c7a] hover:text-[#2d3748] border border-[#e8e0d0] px-2.5 py-1 rounded-lg hover:border-[#2d3748] transition-all">Edit</button>
                {!confirmDelete ? (
                  <button onClick={() => setConfirmDelete(true)} className="text-xs text-[#c0392b] hover:text-[#9b1c1c] border border-[#f5c6c3] px-2.5 py-1 rounded-lg hover:border-[#c0392b] transition-all">Delete</button>
                ) : (
                  <div className="flex items-center gap-1">
                    <button onClick={handleDelete} disabled={deleting} className="text-xs text-white bg-[#c0392b] px-2.5 py-1 rounded-lg hover:bg-[#9b1c1c] transition-all disabled:opacity-50">{deleting ? "..." : "Confirm"}</button>
                    <button onClick={() => setConfirmDelete(false)} className="text-xs text-[#9a8c7a] px-2 py-1 rounded-lg transition-all">Cancel</button>
                  </div>
                )}
              </div>
            )}
            ...
            {isCritiqueAuthor && editing && (
              <button onClick={() => { setEditing(false); setEditError(""); }} className="text-xs text-[#9a8c7a] border border-[#e8e0d0] px-2.5 py-1 rounded-lg transition-all">Cancel</button>
            )}
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-5">
          {editing ? (
            <div className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-sm font-semibold text-[#2d3748]">What's working</label>
                  <span className={`text-xs font-semibold ${editWorkingWords > 0 ? "text-[#b8860b]" : "text-[#b8a898]"}`}>{editWorkingWords} words</span>
                </div>
                <textarea value={editWorking} onChange={(e) => { setEditWorking(e.target.value); setEditError(""); }} rows={5} className="w-full border border-[#2d3748] rounded-xl px-4 py-3 text-sm text-[#2d3748] bg-[#faf7f2] focus:outline-none resize-none leading-relaxed" />
              </div>
              {/* Edit: What needs improvement */}
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-sm font-semibold text-[#2d3748]">What needs improvement</label>
                  <span className={`text-xs font-semibold ${editNeedsWords > 0 ? "text-[#b8860b]" : "text-[#b8a898]"}`}>{editNeedsWords} words</span>
                </div>
                <textarea value={editNeedsWork} onChange={(e) => { setEditNeedsWork(e.target.value); setEditError(""); }} rows={5} className="w-full border border-[#2d3748] rounded-xl px-4 py-3 text-sm text-[#2d3748] bg-[#faf7f2] focus:outline-none resize-none leading-relaxed" />
              </div>
              <p className={`text-xs text-right ${editTotal >= 150 ? "text-[#6b8c6b]" : "text-[#b8860b]"}`}>
                {editTotal} words combined
              </p>
            </div>
          ) : (
          ...
          {editError && (
            <div className="bg-[#fdf1f0] border border-[#f5c6c3] rounded-xl px-4 py-3 text-sm text-[#c0392b]">{editError}</div>
          )}

          {editing && (
            <button onClick={saveEdit} disabled={editSaving} className="w-full py-2.5 bg-[#2d3748] text-white rounded-xl font-semibold text-sm hover:opacity-90 transition-all disabled:opacity-50">
...
export default function FeedbackPage() {
  ...
  useEffect(() => {
    ...
    if (paraMatch) {
      ...
      if (el) { setTimeout(() => { el.scrollIntoView({ behavior: "smooth", block: "center" }); openSidebar(idx); }, 300); }
      ...
      if (idx !== -1 && responseRefs.current[idx]) {
        setTimeout(() => { responseRefs.current[idx].scrollIntoView({ behavior: "smooth", block: "center" }); }, 300);
      }
    }
  ...
  function handleNewComment(data) {
    if (data.type === "reply") {
      setComments((prev) => prev.map((c) => c.id === data.commentId ? { ...c, replies: [...(c.replies ?? []), data.reply] } : c));
    } else {
  ...
  async function handleUpvoteComment(commentId) {
    if (!user) return;
    try {
      const res = await fetch(`${API_URL}/feedback/comments/${commentId}/upvote`, { method: "POST", credentials: "include" });
      if (res.ok) {
        const { upvoted } = await res.json();
        setComments((prev) => prev.map((c) =>
          c.id === commentId
            ? { ...c, upvotes: upvoted ? [{ id: 1 }] : [], _count: { ...c._count, upvotes: (c._count?.upvotes ?? 0) + (upvoted ? 1 : -1) } }
            : c
        ));
      }
    } catch (e) {}
  ...
  async function handleUpvoteResponse(responseId) {
    if (!user) return;
    try {
      const res = await fetch(`${API_URL}/feedback/responses/${responseId}/upvote`, { method: "POST", credentials: "include" });
      if (res.ok) {
        const { upvoted } = await res.json();
        setSubmission((prev) => ({
          ...prev,
          responses: prev.responses.map((r) =>
            r.id === responseId
              ? { ...r, upvotes: upvoted ? [{ id: 1 }] : [], _count: { ...r._count, upvotes: (r._count?.upvotes ?? 0) + (upvoted ? 1 : -1) } }
              : r
          ),
        }));
      }
  ...
  function handleCritiqueEdit(responseId, updatedFields) {
    setSubmission((prev) => ({
      ...prev,
      responses: prev.responses.map((r) => r.id === responseId ? { ...r, ...updatedFields } : r),
    }));
  }
  ...
  // Mark a response as reported locally so the button flips to "Reported" without a refetch
  function handleCritiqueReported(responseId) {
    setSubmission((prev) => ({
      ...prev,
      responses: prev.responses.map((r) =>
        r.id === responseId ? { ...r, reports: [{ id: -1 }] } : r
      ),
    }));
  }
  ...
  // Blind-critique gate: non-authors who haven't critiqued yet can't see others' critiques
  const canSeeCritiques = isAuthor || hasResponded || !user;

  // Min words depends on tier
  const isLongTier  = submission?.wordCountTier === "TIER_4000" || submission?.wordCountTier === "TIER_5000";
  const minWords    = isLongTier ? 300 : 200;
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="bg-white border border-[#e8e0d0] rounded-2xl p-6 mb-6 shadow-[0_2px_8px_rgba(45,35,20,0.04)]">
              <div className="flex flex-wrap items-center gap-2 mb-4">
                <span className="text-[11px] font-semibold text-[#2d3748] bg-[#f4f1ec] px-2.5 py-0.5 rounded-full">{submission.genre}</span>
                <span className="text-[11px] text-[#9a8c7a] border border-[#e8e0d0] px-2.5 py-0.5 rounded-full">{TIER_LABELS[submission.wordCountTier]}</span>
                <span className="text-[11px] text-[#9a8c7a] border border-[#e8e0d0] px-2.5 py-0.5 rounded-full">{DRAFT_LABELS[submission.draftStage]}</span>
                {submission.status === "ARCHIVE" && (
                  <span className="text-[11px] text-[#9a8c7a] bg-[#f4f1ec] border border-[#e8e0d0] px-2.5 py-0.5 rounded-full">Archive · half points</span>
                )}
              </div>

              <h1 className="font-serif text-2xl text-[#2d3748] mb-4 leading-snug">{submission.title}</h1>
              ...
              <p className="text-sm text-[#6b5c4a] mt-5 leading-relaxed border-t border-[#f0ebe3] pt-5">{submission.summary}</p>

              {submission.feedbackWanted?.length > 0 && (
                <div className="mt-5 pt-5 border-t border-[#f0ebe3]">
                  <p className="text-xs font-semibold text-[#9a8c7a] uppercase tracking-wide mb-2">Feedback requested</p>
                  <div className="flex flex-wrap gap-2">
                    {submission.feedbackWanted.map((tag, i) => (
                      <span key={i} className="text-xs text-[#6558d4] bg-[#f2f0fd] px-3 py-1 rounded-full">{tag}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {submission.contentWarnings?.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {submission.contentWarnings.map((w, i) => (
                    <span key={i} className="text-[10px] text-[#c0392b] bg-[#fdf1f0] px-2.5 py-0.5 rounded-full border border-[#f5c6c3]">{w}</span>
                  ))}
                </div>
              )}
            </div>
                ...
                {paragraphs.map((para, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative cursor-pointer transition-all duration-150 rounded-lg -mx-2 px-2 py-1 mb-6 last:mb-0 ${isActive ? "bg-[#fdf9ed]" : "hover:bg-[#fafaf7]"}`}
                      onClick={() => openSidebar(i)}
                    >
                      <div className={`absolute -left-1 top-0 w-0.5 rounded-full transition-all h-full ${isActive ? "bg-[#d4af37] opacity-100" : "opacity-0 group-hover:opacity-100 bg-[#d6cfc4]"}`} />
                      ...
                      {para.html ? (
                        <div className="text-[#2d3748] text-[15.5px] leading-[1.95] tracking-[0.01em] prose-inkwell" dangerouslySetInnerHTML={{ __html: para.content }} />
                      ) : (
                        <p className="font-[Georgia,serif] text-[#2d3748] text-[15.5px] leading-[1.95] tracking-[0.01em]">{renderMarkdown(para.text)}</p>
                      )}
            ...
            <div className="mb-10">
              ...
              {/* ── Blind critique gate ── */}
              {!canSeeCritiques ? (
                <div className="bg-white border border-[#e8e0d0] rounded-2xl px-6 py-10 text-center mb-8">
                  <div className="w-10 h-10 rounded-full bg-[#f4f1ec] flex items-center justify-center mx-auto mb-4">
                    <svg className="w-5 h-5 text-[#9a8c7a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                    </svg>
                  </div>
                  <p className="font-serif text-[#2d3748] mb-1">Critiques are hidden until you write yours</p>
                  <p className="text-sm text-[#9a8c7a] max-w-xs mx-auto leading-relaxed">
                    {(submission.responses?.length ?? 0) > 0
                      ? `There ${submission.responses.length === 1 ? "is" : "are"} ${submission.responses.length} critique${submission.responses.length === 1 ? "" : "s"} waiting. Submit yours below to unlock them.`
                      : "Be the first to leave a critique. Submit yours below."}
                  </p>
                </div>
              ) : (submission.responses?.length ?? 0) === 0 ? (
                <div className="bg-white border border-[#e8e0d0] rounded-2xl px-6 py-12 text-center mb-8">
                  ...
                  <p className="text-sm text-[#9a8c7a]">
                    {canCritique ? "Be the first to leave a full critique below." : "Check back later for community feedback."}
                  </p>
                </div>
              ) : (
                  ...
                  {submission.responses.map((r, rIdx) => (
                    <div key={r.id} ref={(el) => (responseRefs.current[rIdx] = el)}>
                      <CritiqueCard
                        ...
                        onReport={handleCritiqueReported}
                      />
                    </div>
                  ))}
                </div>
              ...
              {canCritique && (
                <CritiqueForm
                  ...
                  minWords={minWords}
                  feedbackWanted={submission.feedbackWanted}
              ...
              {!user && (
                <div className="bg-white border border-[#e8e0d0] rounded-2xl px-6 py-8 text-center">
                  ...
                  <Link to="/login" className="inline-block px-5 py-2.5 bg-[#2d3748] text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-all">Sign in</Link>
                </div>
              )}
